...
void frame_widget::paintEvent(QPaintEvent *p)
{
    ...
    if(ellipse_P) {
        ...
        modified = true;
        ellipse_P = false;
        double time = timer.nsecsElapsed();
        emit sendEllipseTime(time/1000);
    }
...
// first draw...then seed..then change fill color
void frame_widget::boundary_fill()
{
    QRgb edgeColor = qRgb(edgeCol.red(), edgeCol.green(), edgeCol.blue());
